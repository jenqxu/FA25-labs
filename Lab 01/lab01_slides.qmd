...
#| label: read-data
#| echo: true
penguins <- read.csv("penguins.csv", stringsAsFactors = TRUE) #anything you see text, it's not unstructared raw text data, it's categorical data
...
#Notice automatically alphabetical order. 
```
...
# Nick prefers base-R plots, b/c you need to know what you're putting on plots. base R does plots like drawings - we need to put whatever we want. GGPlot is "smart" and adds everything. Make sure we can explain EVERY FEATURE IN PLOTS. 
# Nick usually minimizes package use, b/c packages breaks
# Note that R automatically looks at factors alpha-numeric order. And, c = combine. 
...
```{r, eval = FALSE}
tapply(y, x, FUN)
# y is the thing, x is how you wanna split, FUN is the function
```
...
# take flipper_lengh, split by species, apply mean to each of the splits. 
```
